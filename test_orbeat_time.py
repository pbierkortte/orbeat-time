...
def test_decode_orbeat_time_invalid_code_format():
    ...
    with pytest.raises(ValueError, match="Invalid Orbeat code format: Must be an 8-character string."):
        decode_orbeat_time("1234567", 0)   # Too short
    with pytest.raises(ValueError, match="Invalid Orbeat code format"): # This will be caught by int(..., 8)
...
def test_decode_orbeat_time_not_found_in_window():
    ...
    with pytest.raises(ValueError, match="Could not find a matching timestamp in the search window."):
